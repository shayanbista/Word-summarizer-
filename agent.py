...
from llm import get_openai_model
from fetchFromDb import fetch_chunk_from_db
from conversationHandler import conversation
import json
import re
import os
...
def is_query_informative(query: str) -> bool:
    if not query or len(query.strip()) < 3:
        return False
    trivial_queries = {
        "hi", "hello", "hey", "test", "ok", "okay",
        "what's up", "yo", "hmm", "who are you", "yes", "no",
    }
    return query.strip().lower() not in trivial_queries
...
#                 print("No relevant context found in vector DB.")
#         else:
#             print("Query too generic or informal. Using conversation mode.")
...
#         else:
#             # Fallback to conversation chain (memory-enabled)
...
#     print("context to be sent", user_input)
#     try:
#         llm = get_openai_model()  # For direct invoke calls
#         context = None
#         chunks = None
...
#             response_text = response.content.strip()
#         else:
#             # Fallback to conversation chain with memory
#             response_text = conversation.predict(input=user_input)
...
def handle_user_input(user_input):
    ...
    try:
        ...
        if context:
            # Use context but also push conversation into memory
            system_message = (
                "You are a highly specialized research assistant.\n"
                "- Use the research context provided.\n"
                "- Cite context when answering.\n"
            ...
            conversation.memory.chat_memory.add_ai_message(response_text)
        else:
            # Fallback to conversation chain with memory
            response_text = conversation.predict(input=user_input)
